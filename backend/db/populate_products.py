...
conn = sqlite3.connect(Path(__file__).with_name('ecommerce.sqlite'))
cur = conn.cursor()
...
''')
cur.executemany('INSERT OR IGNORE INTO products (name, description, price, category, image_url, stock) VALUES (?, ?, ?, ?, ?, ?)', products)
conn.commit()
print('Products now in database:', cur.execute('SELECT COUNT(*) FROM products').fetchone()[0])
conn.close()
